...
font c = convertToPixel $ (recombine (mapToBinary $ ascii c))
    where   
        convertToPixel xs = Pixel {mensaje = map asterisk xs}
            where
                asterisk xs = foldr (\a b -> (if a == 0 then ' ' else '*'):b) [] xs

        recombine ([]:_) = []
        recombine xs = (heads xs) : (recombine $ tails xs)
            where
                heads [] = []
        ...
        mapToBinary xs = map binary xs
            where 
                binary n = (twoBinary n) ++ (zeroes n)
                    where   
                        zeroes n = take (7 - (length $ twoBinary n)) [0,0,0,0,0,0,0]
                        twoBinary 0 = []
                        twoBinary n = (mod n 2):twoBinary (div n 2)
        ascii c 
